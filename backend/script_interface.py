...
def get_args():
    ...
    parser.add_argument("--config_json", "-c", help='Json file with parameters for analysis',
                        default="config_files/config_file.json")
    ...
    parser.add_argument("--max_na_percent_protein", "-nap", type=int, help="remove protein with less than max_na_percent")
...
if __name__ == '__main__':
    ...
    if args.update_config_file:
        init_fonction.write_config_file(input=args.json_template, organism=args.organism, group=args.group,
                                        max_na_prot=args.max_na_percent_protein, max_na_sample=args.max_na_percent_sample,
    ...
    if args.run:
        ...
        target = {"quality_check": "no_na", "analysis": "ttest"}
        rerun = str(args.rerun)

        # TODO stdout/stderr
        # Venv + snakemake
        p = subprocess.Popen([path_to_script, '-i', args.analysis_ID, '-t', target[args.step], '-r', rerun], shell=False)
        p.wait()
        p.terminate()
