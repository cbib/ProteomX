...
def keep_specific_proteins_na(df, filter, step, col_name, threshold_specific=20):
    """
    ...
    # create mask :
    # true if one of the condition has 100% missing values (default value) and the other none : specific strict
    mask = (subset_df == 100).any(axis=1) & (subset_df < threshold_specific).any(axis=1)
...
def keep_specific_proteins_na_same_threshold(df, filter, name):
    """
    Assumption : only 2 conditions
    Old threshold for polyphenolprot/higherflavo
    """
    # filter nan percentage info columns
    subset_df = df.filter(regex=filter)
    ...
    mask = (subset_df == 100).any(axis=1) & (subset_df < 34).any(axis=1)
...
def remove_flagged_samples(df: pd.DataFrame, boolean_mask, metadata_col, values_col_prefix, keep_specific,
    ...
    not_data_col = df[df.columns.drop(list(df.filter(regex=values_col_prefix)))]
    data = df.filter(regex=values_col_prefix)

    res = data.loc[:, ~boolean_mask]
    if keep_specific:
        return pd.concat([not_data_col, res, df[col_name]], axis=1)
    else:
        return pd.concat([not_data_col, res], axis=1)


def replace_values_to_nan(data_df: pd.DataFrame, replace_values: dict, values_cols_prefix: str) -> pd.DataFrame:
    ...
    op = replace_values["operator"]

    # select columns with values
    val = data_df.filter(regex=values_cols_prefix)
    not_val = data_df.drop(data_df.filter(regex=values_cols_prefix).columns, axis=1)

    # replace all values defined by (operator, value) with NaN
    val[ops[op](val, value)] = np.nan

    # result data frame
    res = pd.concat([not_val, val], axis=1)
    return res
