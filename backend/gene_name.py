...
def get_args():
    ...
    parser.add_argument("--config_file", "-c", help='json file with parameters for analysis')
...
def add_gene_name_gprofiler(data_df: pd.DataFrame, col: str, organism: str) -> pd.DataFrame:
    ...
    res_f = res_f.replace({'UNIPROTSWISSPROT': 'Swiss-Prot','UNIPROTSPTREMBL': 'TrEMBL'})
...
def add_gene_name_annotation_files(data_df, col, organism):
    ...
    if organism == "mmusculus":
        ...
        swiss_df = swiss_df.rename({'Swiss': col, 'Gene names': 'gene_name_swiss'}, axis=1)
        swiss_df = swiss_df[[col, 'gene_name_swiss']]
        trembl_df = trembl_df.rename({'Trembl': col, 'Gene names': 'gene_name_trembl'}, axis=1)
        trembl_df = trembl_df[[col, 'gene_name_trembl']]

        res_gene_name = genename_proteins(data_df, col, swiss_df, trembl_df, fragment)

    elif organism == "hsapiens":
        logger.debug('Organism : hsapiens')
        swiss_file = os.path.join(paths.global_data_dir, 'annotation_data/hsapiens/210209_UniProt_SwissProt_hsapiens.tab')
        trembl_file = os.path.join(paths.global_data_dir, 'annotation_data/hsapiens/210209_UniProt_Trembl_hsapiens.tab')
        ...
        swiss_file = os.path.join(paths.global_data_dir, 'annotation_data/scerevisiae/uniprot_swiss_prot_scerevisiae.tab')
        trembl_file = os.path.join(paths.global_data_dir, 'annotation_data/scerevisiae/uniprot_trembl_scerevisiae.tab')
        ...
        swiss_file = os.path.join(paths.global_data_dir, 'annotation_data/rnorvegicus/uniprot_swiss_prot_rnorvegicus.tab')
        trembl_file = os.path.join(paths.global_data_dir, 'annotation_data/rnorvegicus/uniprot_trembl_rnorvegicus.tab')
...
if __name__ == "__main__":
    ...
    logpath = os.path.join(paths.global_data_dir, args.file_id, 'log/gene_name.log')
    ...
    functions = {'gProfiler': add_gene_name_gprofiler,
                 'annotations_files': add_gene_name_annotation_files}
