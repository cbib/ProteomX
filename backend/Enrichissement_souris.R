...
input_df = input_df[input_df$gene_name_bank == 'swissprot', ]
...
CC_mouse <- enrichGO(gene          = gene_list,
                      OrgDb         = org.Mm.eg.db,
                      keyType       = 'SYMBOL',
                      ont           = "CC",
                      pAdjustMethod = "BH",
                      pvalueCutoff  = 0.05,
                      qvalueCutoff  = 0.2)
plot_CC <- dotplot(CC_mouse,showCategory=10)
...
MF_mouse <- enrichGO(gene          = gene_list,
                     ...
                     pvalueCutoff  = 0.05,
                     qvalueCutoff  = 0.2)
plot_MF <- dotplot(MF_mouse,showCategory=10)
png(filename = output_name_MF, res = 300, units  = 'in', width = 10,
    height = 7)
print(plot_MF)

mf_df <- data.frame(MF_mouse)
print('---')
...
BP_mouse <- enrichGO(gene          = gene_list,
                     OrgDb         = org.Mm.eg.db,
                     keyType       = 'SYMBOL',
                     ont           = "BP",
                     pAdjustMethod = "BH",
                     pvalueCutoff  = 0.05,
                     qvalueCutoff  = 0.2)
plot_BP <- dotplot(BP_mouse,showCategory=10)
png(filename = output_name_BP, res = 300, units  = 'in', width = 10,
    height = 7)
print(plot_BP)

bp_df <- data.frame(BP_mouse)
